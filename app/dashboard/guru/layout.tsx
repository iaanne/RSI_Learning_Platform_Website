    ...
    <div className="flex min-h-screen bg-[#F8FAFC]">
      ...
      <aside className="w-72 bg-slate-900 text-slate-300 hidden md:flex flex-col fixed h-full shadow-2xl">
        ...
        <div className="p-6 border-t border-slate-800/50">
          <Link href="/auth/login" className="w-full flex items-center space-x-3 text-slate-500 hover:text-rose-400 p-4 rounded-2xl transition-all hover:bg-rose-500/5">
            <LogOut size={20} />
      ...
      <main className="flex-1 md:ml-72 flex flex-col min-h-screen">
        <header className="bg-white/80 backdrop-blur-md border-b border-slate-100 px-8 py-5 flex justify-between items-center sticky top-0 z-30">
          <div className="flex items-center space-x-2 text-slate-400">
             ...
             <span className="text-xs font-bold uppercase tracking-widest">Dashboard • {pathname.split('/').pop()?.replace('-', ' ')}</span>
          </div>
